...
def evaluate_window_pll(model, filled_text, mask_char_pos, window_size=PLL_CONTEXT_WINDOW):
    ...
    if seq_len <= window_size * 2 + 2:
        score = model.evaluate_mlm_score_fast(filled_text)
        eval_count = sum(
    ...
    # フォールバック: 文字位置の比率からトークン位置を推定
    if mask_token_pos is None:
        ratio = mask_char_pos / max(len(filled_text), 1)
        mask_token_pos = max(1, min(int(ratio * seq_len), seq_len - 2))
