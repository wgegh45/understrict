...
def build_ensemble_combined_output(results_by_method,
    ...
    for idx in range(num_masks):
        ...
        for rank in range(max_rank):
            ...
            yield rank, r_cell, p_cell, h_cell, idx
...
def predict_masks_per_window(model, text, jamo_info=None, use_full_context=True):
    ...
    for mask_idx, mask_pos in enumerate(mask_positions_in_tokens):
        ...
        with torch.no_grad():
            try:
                import inspect
                forward_params = inspect.signature(model.model.forward).parameters
...
def calculate_ensemble_scores_rank(all_model_predictions, model_names):
    ...
    num_masks = min(pred_lengths) if pred_lengths else 0
    ensemble_results = []
...
def main():
    ...
    for line_num, line in enumerate(lines, 1):
        ...
        for mask_type_key in custom_types_in_line:
            ...
            if mask_type_key not in mask_groups:
                mask_groups[mask_type_key] = []
            mask_groups[mask_type_key].append({
                'line_num': line_num,
    ...
    for mask_type, entries in mask_groups.items():
        ...
        # ============================================================
            ...
            if len(entries) >= 2 or total_masks >= 2:
                ...
                # PLL評価用モデルの選択
                eval_model_name = 'klue/roberta-large'
