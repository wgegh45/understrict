...
# PLL設定
PLL_ALPHA = 0.6         # PLLスコアの重み
PLL_BETA = 0.4          # 確率スコアの重み（1-alphaが目安）
PLL_TEMPERATURE = 0.2   # ソフトマックス温度（小さいほど上位集中）
PLL_CONTEXT_WINDOW = 50 # ウィンドウPLLの評価トークン幅（MASK位置の前後）
...
class KoreanMaskCompletion:
    def __init__(self, model_path, top_k=10):
        ...
        # Token IDキャッシュ構築
        tokens_list = self.tokenizer.convert_ids_to_tokens(
...
def evaluate_window_pll(model, filled_text, mask_char_positions, window_size=PLL_CONTEXT_WINDOW):
    """MASK位置周辺のウィンドウでPLLスコアを計算"""
    ...
    if offsets is None or not mask_token_positions:
        for mpos in mask_char_positions:
            ratio = mpos / max(len(filled_text), 1)
            estimated = max(1, min(int(ratio * seq_len), seq_len - 2))
            mask_token_positions.append(estimated)
    ...
    start = max(1, center_pos - window_size)
    end = min(seq_len - 1, center_pos + window_size + 1)
            ...
            if len(entries) >= 2 or total_masks >= 2:
                ...
                for token, data in all_mask_candidates.items():
                    if not KoreanMaskCompletion.is_valid_korean_token(token):
                        continue
                    max_prob = max(p for _, p in data['details'])
                    if max_prob < 0.01:
                        continue
                ...
                # ===== ステップ3: 字母フィルタリング =====
                # エントリ内の字母情報を収集
                jamo_patterns_for_type = []
                for entry in entries:
                    for ji in entry.get('jamo_info', []):
                        jamo_patterns_for_type.append(ji[1])
                
                if jamo_patterns_for_type:
                    jamo_filtered = {}
                    for token, data in candidate_pool_filtered.items():
                        if any(match_jamo_pattern(token, jp) for jp in jamo_patterns_for_type):
                            jamo_filtered[token] = data
                    if jamo_filtered:
                        candidate_pool_filtered = jamo_filtered
                
                # ===== ステップ4: 行別PLLを計算し、キャッシュ =====
                # PLLは文脈適合性（その文に自然か）を測り、確率は語彙頻度（モデルがよく予測する語か）を測る
                # 文脈をより重視したい場合（例：同じMASKが複数行にまたがる場合）は alpha を上げます
                # 語彙の一般的な自然さを重視したい場合は beta を上げます（0.5〜0.6）。
                # 学術的な慣例 0.6/0.4 や 0.7/0.3 が「とりあえず試す値」
                alpha = PLL_ALPHA
                beta = PLL_BETA
                temperature = PLL_TEMPERATURE
                ...
                for entry_idx, entry in enumerate(entries):
                    ...
                    for token, data in candidate_pool_filtered.items():
                        ...
                        pll_score, eval_count = evaluate_window_pll(
                            eval_model, filled, adjusted_positions
                        )
